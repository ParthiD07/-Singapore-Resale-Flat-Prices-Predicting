...
class columns():
    ...
    flat_model=['IMPROVED', 'NEW GENERATION', 'MODEL A', 'STANDARD', 'SIMPLIFIED','MODEL A-MAISONETTE', 'APARTMENT', 'MAISONETTE', 'TERRACE',
       '2-ROOM', 'IMPROVED-MAISONETTE', 'MULTI GENERATION','PREMIUM APARTMENT', 'ADJOINED FLAT', 'PREMIUM MAISONETTE',
...
if menu == 'Flat Resale Price Prediction':
    ...
    with st.form('price_prediction'):
        ...
        with col1:
            ...
            Story_Starts_at=st.slider('**Story_Starts_at**', min_value=1, max_value=100)
        ...
        with col2:
            ...
            Story_Ends_at=st.slider('**Story_Ends_at**', min_value=1, max_value=100)
        ...
        if button:
            if not all([Year,Flat_Type,Town,Story_Starts_at,Lease_Commence_Year,Flat_Model,Floor_Area_sqm,Story_Ends_at]):
                st.error('Please fill in all the required fields.')
            else:
                with open ('Resale_price_predict_Linear_model.pkl','rb') as file:
            ...
            user_input=np.array([[Year, Town_encoded, Flat_Type_encoded, Flat_Model_encoded, Floor_Area_sqm, Story_Starts_at, Story_Ends_at, Lease_Commence_Year]])
            # Scale the input values
            scaled_input=scaler.transform(user_input)
            # Make predictions using the trained model
            predict=model.predict(scaled_input)
            st.markdown(f'<h4 style=color:#fbe337>The Predicted Flat Resale Price is: {predict[0]:.2f} SGD</h4>',unsafe_allow_html=True)
            st.snow()
